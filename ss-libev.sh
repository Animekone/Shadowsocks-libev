...
check_status(){
	status=`systemctl status ss-libev | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1`
}
...
Uninstall(){
    ...
    if [ "${answer}" == "y" ] || [ "${answer}" == "Y" ]; then
        ps -ef | grep -v grep | grep -i "ss-libev" > /dev/null 2>&1
        systemctl stop ss-libev
        systemctl disable ss-libev
        rm -fr /etc/ss-libev
